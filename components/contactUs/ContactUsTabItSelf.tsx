...
import {singleContactUsMessageSchema} from '../../types';
...
interface schema {
  item: singleContactUsMessageSchema;
  id: string;
  isInTrash?: boolean;
...
const OrderItSelf = ({
  ...
  const {name: displayName, time, email, shouldMark} = item;
  ...
  const onDelete = () => {
    ...
    onPrimaryDelete({
      collection: !isInTrash ? 'contact-us' : 'trash',
      id,
      item,
            ...
            <View style={styles.flexBox}>
              <View style={{flex: 1}}>
                <View style={[styles.flexBox, styles.flexBoxJustify]}>
                  ...
                  {isInTrash && <Premium title="Contact Us" />}
                </View>
