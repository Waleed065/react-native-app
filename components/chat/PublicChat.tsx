...
import {getPublicChats} from '../../STORE/selectors';
...
export default function Chat(): JSX.Element {
  ...
  const chats = useSelector(getPublicChats);
  ...
  const onChatPress = useCallback(chatId => {
    navigation.navigate(stackNavigation.chatRoom, {chatId, withAdmin: false});
  }, []);
    ...
    <View
      ...
      <View style={[styles.flatListStyle, {height: screenHeight}]}>
        {Boolean(dataProvider._data.length) ? (
          ...
          <NotAvailable title={'There are currently no public chats 😕'} />
        )}
      </View>
    </View>
  );
